    ...
    public class VentaController : ControllerBase
    {
        ...
        public async Task<IActionResult> AnularVenta(int idVenta, [FromBody] AnularVentaDto anularVentaDto)
        {
            ...
            try
            {
                ...
                return Ok(new { StatusCode = 200, message = "Registro actualizado con éxtio", data = _mapper.Map<VentaDto>(ventaAnulada) });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<IEnumerable<DetalleVentaDto>>> GetDetallesByIdVenta(int idVenta)
        {
            try
            {
                var entidad = await _ventaService.GetDetallesByIdVenta(idVenta);
                if (entidad.Count == 0 || entidad == null)
                {
                    return NotFound(new { StatusCode = 404, message = "No se encuentran detalles para la venta espeficicada." });
                }
